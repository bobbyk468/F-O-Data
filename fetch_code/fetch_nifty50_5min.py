#!/usr/bin/env python3
"""
Fetch 5-minute OHLC data for Nifty 50 for max possible range (2015-09-01 to today).
Uses chunked requests (API returns max 100 candles per call). 5min = ~75 bars/day,
so we use 1-day chunks to stay under limit.
...
# ~75 five-min bars per trading day; 100 bars ≈ 1.3 days
CHUNK_DAYS = 1


def main():
    parser = argparse.ArgumentParser(description="Fetch Nifty 50 5min data (max range by default)")
    ...
    # Sort by date and deduplicate by date
    by_ts = {}
    ...
    base = str(REPO_ROOT)
    out_dir = os.path.join(base, "data", "indices", "5min")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "nifty_50_5min.csv")
