...
def main() -> int:
    ...
    if not args.skip_login:
        steps.append(Step("login (test_login.py)", [*py_u, "fetch_code/test_login.py"]))

    steps.extend(
        [
            Step("indices 15min", [*py_u, "fetch_code/fetch_all_indices_15min.py", "--workers", str(args.workers)]),
            Step("nifty 50 15min", [*py_u, "fetch_code/fetch_nifty50_15min.py"]),
            Step("F&O stocks 15min", [*py_u, "fetch_code/fetch_fo_stocks_15min.py", "--workers", str(args.workers)]),
        ]
    )
    ...
    if args.eod_full_history:
        steps.extend(
            [
                Step(
                    "indices EOD (full history)",
                    [
                        *py_u,
                        "fetch_code/fetch_eod_90d.py",
                        "--only",
                        "indices",
                        "--full-history",
                        "--workers",
        ...
        steps.extend(
            [
                Step("indices EOD (last 90d)", [*py_u, "fetch_code/fetch_eod_90d.py", "--only", "indices", "--days", "90", "--workers", str(args.workers)]),
                Step("F&O EOD (last 90d)", [*py_u, "fetch_code/fetch_eod_90d.py", "--only", "fo", "--days", "90", "--workers", str(args.workers)]),
            ]
        )
