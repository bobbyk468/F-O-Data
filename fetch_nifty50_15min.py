#!/usr/bin/env python3
"""
Fetch 15-minute OHLC data for Nifty 50 for max possible range (2015-09-01 to today).
Uses chunked requests (API returns max 100 candles per call) and saves to CSV.
...
# Max possible range: Zerodha 15min index data from 2015-09-01
DEFAULT_START_DATE = date(2015, 9, 1)
# ~25 fifteen-min bars per trading day; 100 bars ≈ 4 days
CHUNK_DAYS = 4


def main():
    parser = argparse.ArgumentParser(description="Fetch Nifty 50 15min data (max range by default)")
